...
**Markdown and color:** Kibana sanitizes styled HTML in markdown panels, so the governance dashboard avoids `<div style=…>` heroes and **does not rely on `![](data:image/…)`** (those links often show as raw text). It uses **emoji** and **emoji in panel titles** for color that survives sanitization.
