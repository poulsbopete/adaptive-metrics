...
| `dashboards/instruqt-metric-governance-dashboard.json` | **Instruqt** push (**`scripts/push_governance_dashboard.py`**): same root contract as a **Systems Operations**–style export (**`markdown`**, **`vis`**, **`options`**, **`refresh_interval`**). **ES|QL** panels are **`vis` + `data_table` + `data_source.type: esql`** — on **2023-10-31** that API does **not** accept **`vis` + `metric` + `esql`** (native **metric**/**xy** tiles use **`data_view_reference`** + aggregations instead). |
